...
export default function Tasks() {
  ...
  return (
    <Box sx={{ width: '100%' }}>
        ...
        PaperProps={{
          ...
          onSubmit: (event) => {
            ...
            if (!editMode) {
                ...
                setTasks(tempTasks.map((task, index) => {
                    return {...task, id: index};
                }));
                localStorage.setItem('tasks', JSON.stringify(tasks));
      ...
      >
        <DialogTitle>{editMode ? 'Редактирование' : 'Создание'} Задачи</DialogTitle>
        <DialogContent>
          <TextField
            required
            id="title"
            ...
            fullWidth
            variant="standard"
            defaultValue={tasks.find(task => task.id == selected[0])?.title}
          />
          <TextField
            required
            id="description"
            ...
            fullWidth
            variant="standard"
            defaultValue={tasks.find(task => task.id == selected[0])?.description}
          />
          <TextField
            required
            id="project"
            ...
            label="Проект"
            variant="standard"
            defaultValue={tasks.find(task => task.id == selected[0])?.project}
          />
          <TextField
            focused
            id="endDate"
            ...
            variant="standard"
            type="date"
            defaultValue={tasks.find(task => task.id == selected[0])?.endDate}
          />
        </DialogContent>
        <DialogActions>
